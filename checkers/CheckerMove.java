...
public class CheckerMove {
    ...
    /*
    noMovesLeft returns true if no more movements are left for the next player
...
//"Apply move" in the Minimax. Simply moves the board given moves.
     ...
     for (int i=-2; i<=2; i+=4)
       for (int j=-2; j<=2; j+=4)
         if (inRange(newx+i,newy+j)) {
            ...
            if (moveResult == legalMove) {      // An ordinary move without additional.
